...
| 8 | **Security baseline** (auth+MFA, secrets mgmt, encryption, dep scanning, tested backups, incident-response plan) | ◑ Partial: Supabase auth, RLS (security-advisor clean), service-role key server-only, TLS + at-rest encryption, **Dependabot now enabled** (2026-09-01) | ⚠️ **Add: MFA option, a *tested* backup restore, and fill counsel/insurer contacts into the incident-response plan** |
