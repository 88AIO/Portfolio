...
| 4 | **Age gate + under-13 block, logged** | ✅ 18+ stated in Terms/Privacy; signup checkbox blocks submission until checked; **now logged** — `consent_log` records `min_age_confirmed`/`agreed_terms_and_privacy` with a server timestamp per signup (2026-09-01) | — no under-13 collection path exists |
...
| 8 | **Security baseline** (auth+MFA, secrets mgmt, encryption, dep scanning, tested backups, incident-response plan) | ✅ Supabase auth, RLS (security-advisor clean), service-role key server-only, TLS + at-rest encryption, Dependabot enabled, **optional TOTP 2FA shipped** (2026-09-01, `components/MfaSection.tsx` + a server-side `proxy.ts` check — see git log for the "why gate in middleware, not just the login page" reasoning) | ⚠️ A *tested* backup restore, and fill counsel/insurer contacts into the incident-response plan |
