...
- Errors report to Sentry (org `88aio`, project `snowfolio`) from every runtime via `instrumentation.ts` / `instrumentation-client.ts`; the DSN is committed in `lib/observability.ts` (public by nature, env override or `""` to disable). Session replay and tracing are deliberately off — replay would record someone's holdings. The nightly sync checks in to a Sentry cron monitor (`nightly-sync`), which alerts on a missed night without any outside caller reaching the site.
